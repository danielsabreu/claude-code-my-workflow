# SDW public
#sdw_pub <- get_data_with_password("RESR.Q.PT._T.N._TR.TVAL.4D0.TB.N.IX",iam_user=iam_user,iam_pass=iam_pass)

#SDW restricted
#sdw_res <- get_data_with_password("BSI.M..N.A.A20.F.1.U6.2250.Z01.E",iam_user=iam_user,iam_pass=iam_pass)
...
get_dataflows <- function(...) {
  ...
  query_url <- "https://esb-ws-scde.eu.escb.eu/ws/SDW21/REST/service/dataflow"
...
get_data_with_password <- function(key, filter = NULL, iam_user, iam_pass, ...) {
  ...
  req <- make_request_with_password(query_url, "data", iam_user=iam_user, iam_pass=iam_pass, ...)
...
create_query_url <- function(key, filter = NULL) {
  ...
  url <- "https://esb-ws-scde.eu.escb.eu/ws/SDW21/REST/service/data"
...
make_request_with_password <- function(query_url, header_type, iam_user, iam_pass, ...) {
  ...
  req <- httr::GET(query_url, httr::add_headers(
    "Accept" = accept_headers[header_type],
    "Accept-Encoding" = "gzip, deflate"),
    httr::authenticate(user=iam_user,password =iam_pass),...)
