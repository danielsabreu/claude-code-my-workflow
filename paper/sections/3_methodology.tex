...
Following \citet{bai2003inferential}, as $(N,T)\to\infty$ with $\sqrt{N}/T\to 0$:
\begin{equation}
    ...
    \label{eq:factor_clt}
\end{equation}
where $\boldsymbol{H}$ is the rotation matrix arising from PCA's rotational indeterminacy, $\boldsymbol{V}$ is the diagonal matrix of the $r$ largest eigenvalues of $(NT)^{-1}\boldsymbol{X}\boldsymbol{X}'$, and $\boldsymbol{\Gamma}_t$ summarises the cross-sectional dependence structure of the idiosyncratic components at time $t$.

The feasible asymptotic variance is $\widehat{\mathrm{Avar}}(\hat{\boldsymbol{f}}_t) = \boldsymbol{V}_{NT}^{-1}\hat{\boldsymbol{\Gamma}}_t\boldsymbol{V}_{NT}^{-1}$, where:
\begin{equation}
    \hat{\boldsymbol{\Gamma}}_t = \frac{1}{n}\sum_{i=1}^{n}\sum_{j=1}^{n} \hat{\boldsymbol{\lambda}}_i\hat{\boldsymbol{\lambda}}_j'\,\frac{1}{T}\sum_{s=1}^{T} \hat{e}_{is}\hat{e}_{js},
    \label{eq:gamma_est}
\end{equation}
with $\hat{e}_{it} = x_{it} - \hat{\boldsymbol{\lambda}}_i'\hat{\boldsymbol{f}}_t$ and $n = \min\!\bigl\{\lfloor N^{1/2}\rfloor,\lfloor T^{1/2}\rfloor\bigr\}$. Pointwise confidence bands for the $k$-th factor follow directly as $\hat{f}_{k,t} \pm z_{\alpha/2}\bigl[\widehat{\mathrm{Avar}}(\hat{\boldsymbol{f}}_t)_{kk}/N\bigr]^{1/2}$.
