...
Two journals commonly targeted by empirical macrofinance and macroprudential policy papers.

### International Journal of Central Banking (IJCB)

**Short name:** `IJCB`

**Focus.** Monetary policy, financial stability, macroprudential policy, systemic risk measurement, central banking operations. Sponsored by the BIS and a consortium of central banks. Accepts DFM/factor models, VAR-based approaches, and descriptive indicator work as mainstream methods — no need for causal identification. Policy hook is load-bearing.

**Bar.** Results must speak to a policy instrument (CCyB, DSTI ratios, monetary policy stance, etc.) or to a central bank monitoring task. Pure methodology without a financial-stability angle does not clear the desk. Comparison with established indicators (credit-to-GDP gap, BIS EWS, ECB indicators) is expected.

**Domain-referee adjustments.**
- Contribution 30 → 25 (measurement contribution accepted without identification; bar on novelty vs existing indicators is the binding constraint)
- External validity 15 → 20 (cross-country applicability matters — does it work beyond the estimation sample?)
- Fit 10 → 15 (policy fit is a positive signal, not just a check-box)

**Methods-referee adjustments.**
- Identification 35 → 10 (IJCB does not require causal identification for indicator/measurement papers)
- Robustness 15 → 30 (factor selection, country coverage, vintage sensitivity, bandwidth choice — this is the workhorse concern)
- For `descriptive` / `reduced-form` papers: Replication 5 → 15 (data and code availability increasingly expected; BIS working paper companion is common)

**Typical concerns.**
- "How does this compare to the BIS credit-to-GDP gap / ESRB EWS indicators?"
- "How sensitive are the results to the choice of variables in the factor model?"
- "Is the out-of-sample early-warning performance evaluated with real-time data, or ex post?"
- "What would a macroprudential authority do differently if they adopted this indicator?"
- "Does the neutral risk threshold hold across different country contexts?"
...
**Table format override.** Coefficients with SE in parentheses; significance stars optional but common. No specific style guide — follow the paper's own consistent convention.

---

### Economic Policy (Oxford)

**Short name:** `Economic Policy`

**Focus.** Policy-relevant empirical economics with EU/OECD scope. Broad readership (professional economists + policymakers). Published biannually with a panel discussion format: each paper is accompanied by a discussant comment and author response. The non-technical summary ("Panel introduction") is part of the accepted manuscript and is required.

**Bar.** "The policy implication must be statable in one sentence to a non-specialist." Methods must be sound but the journal rewards accessibility and breadth of relevance over technical depth. Pure methodology is a desk rejection unless the policy application is the center of the paper.

**Domain-referee adjustments.**
- Contribution 30 → 20 (novelty bar is lower; policy impact bar is higher)
- External validity 15 → 25 (generalizability beyond one country / one period is load-bearing)
- Substance 20 → 20 (accessible argument over formal argument)
- Fit 10 → 15 (EU/OECD policy relevance is a positive signal)

**Methods-referee adjustments.**
- Identification 35 → 20 (robustness and transparency valued over identification purity)
- Robustness 15 → 25 (are the results stable enough to advise a policymaker with?)
- For `descriptive` papers: this is a first-class paper type at Economic Policy; no penalties for "only measurement."

**Typical concerns.**
- "What should a finance minister / central bank governor do differently based on this?"
- "Is the 'so what' answerable in plain English to a policymaker who won't read Section 4?"
- "Are results robust across the EU member states the paper doesn't study directly?"
- "Is the non-technical summary genuinely non-technical, or does it smuggle in jargon?"
- "Is there a practical implementation path, or only a diagnosis?"
...
**Table format override.** Accessible format; footnotes for technical details; no prescribed style — Oxford author guidelines apply at submission.
